...
async function run() {
  try {
    ...
    // get top selling 6 food item for the topfood section
    app.get('/api/v1/route/topfood', async(req, res) => {
      const cursor = allFoodCollection.find().sort({price: 'desc'}).limit(6);
      const result = await cursor.toArray() ;
      res.send(result)
    })
